...
import { renderForms } from "../authForm/authForm";
...
export default function OpenMainMenu(e) {
  ...
  btnMainMenu.classList.add("duration-500");
...
export function OpenMenuAside(e,id) {
  e.preventDefault();
  
  let liMenuAside = document.getElementById(id);
  let itemMenuAside = document.querySelectorAll('.item-menuAside');
  console.log(itemMenuAside)
  //liMenuAside.forEach((item) => {
  let  test = liMenuAside;
  let  test2 = test.nextElementSibling;
   //-test 3 baraye menu sath 2
  let  test3 = test2.children;

   //----------hide menu sath 1------item haye menu asli amlak o khodro ...
    for (let j = 0; j < itemMenuAside.length; j++) {
      itemMenuAside[j].classList.add("hidden");
    }
...
export function OpenMenuAside2(e) {
  e.preventDefault();
  //e.stopPropagation()
...
export function BtnLogin(e) {
  ...
  btnLogin.classList.add("bg-blue-400", "text-white");
  btnLogin.previousElementSibling.classList.remove(
    "bg-blue-400",
    "text-white"
  );
  btnLogin.previousElementSibling.classList.add("text-slate-700");
}
export function BtnRegister(e) {
  e.preventDefault();
  const authForm = document.getElementById("auth-form");
  ...
  btnRegister.classList.add("bg-blue-400", "text-white");
  btnRegister.nextElementSibling.classList.remove("bg-blue-400", "text-white");
  btnRegister.nextElementSibling.classList.add("text-slate-700");
}
